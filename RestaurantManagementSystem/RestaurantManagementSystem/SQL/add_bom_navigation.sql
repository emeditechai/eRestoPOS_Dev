-- =============================================
-- Script : add_bom_navigation.sql
-- Purpose: Add "BOM Configure" child page under the
--          existing "Stocks" top-level navigation.
...
VALUES
    ('NAV_STOCKS_BOM',  'NAV_STOCKS',  'BOM Configure',
     'Bill of Material – Link Menu Items to Ingredient Master with cost & margin',
     NULL, 'BOM', 'BOMList', NULL, NULL,
     'fas fa-layer-group compact-icon text-success',
     5, 1, 1,
     NULL, NULL, 0);
...
PRINT 'BOM Configure navigation entry merged.';
...
BEGIN
    ...
    WHERE nm.Code = 'NAV_STOCKS_BOM'
      AND NOT EXISTS (
          SELECT 1 FROM dbo.RoleMenuPermissions rmp
          WHERE rmp.RoleId = @AdminRoleId AND rmp.MenuId = nm.Id
      );

    PRINT 'Administrator role permissions granted for BOM Configure.';
END
ELSE
...
WHERE  Code IN ('NAV_STOCKS', 'NAV_STOCKS_UOM', 'NAV_STOCKS_BOM')
